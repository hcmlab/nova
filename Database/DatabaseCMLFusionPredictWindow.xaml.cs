...
namespace ssi
{
    ...
    public partial class DatabaseCMLFusionPredictWindow : Window
    {
        ...
        List<SchemeTrainerPair> fusionstreams = new List<SchemeTrainerPair>();


        public DatabaseCMLFusionPredictWindow(MainHandler handler, Mode mode)
        {
            InitializeComponent();
            ...
            switch (mode)
            {
                ...
                case Mode.PREDICT:
                    ...
                    Title = "Fuse Model Predicitons";
                    ApplyButton.Content = "Fuse";
        ...
        private void Apply_Click(object sender, RoutedEventArgs e)
        {
            ...
            List<SchemeTrainerPair> items = new List<SchemeTrainerPair>();

            foreach(SchemeTrainerPair item in SchemeandClassifierBox.SelectedItems)
            {
                items.Add(item);
            ...
            string streamsList = "";
            string schemesList = "";
            string annotatorsList = "";
            string trainersList = "";



            foreach (SchemeTrainerPair item in items)
            {
                if (streamsList == "")
                {
                    streamsList += item.Stream;
                }
                else
                {
                    streamsList += ";" + item.Stream;
                ...
                || mode == Mode.COMPLETE)
            {
                if (true || force)
                {
                  ...
                  string outputprediction = NetworkBox.SelectedItem.ToString();

                  string netpath = Properties.Settings.Default.CMLDirectory + "\\" +
                  Defaults.CML.FusionFolderName + "\\" +
                  Defaults.CML.FusionBayesianNetworkFolderName + "\\" + outputprediction;




                    logTextBox.Text += handler.CMLPredictFusion(trainer.Path,
                        Properties.Settings.Default.DatabaseDirectory,
                        Properties.Settings.Default.DatabaseAddress,
                        Properties.Settings.Default.MongoDBUser,
                        MainHandler.Decode(Properties.Settings.Default.MongoDBPass),
        ...
        private void GetStreams()
        {
            ...
            foreach (DatabaseStream stream in streams)
            {
                foreach (DatabaseScheme scheme in schemes)
                {
                    ...
                    if (getTrainer(stream, scheme, false).Count > 0)
                    {
        ...
        private void StreamsBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (StreamsBox.SelectedItem != null)
            {
                ...
                if (scheme != null)
                {
                    ...
                    List<string> networks = getBayesianNetworks();
        ...
        private void AddItemButton_Click(object sender, RoutedEventArgs e)
        {
              ...
              SchemeTrainerPair stp = new SchemeTrainerPair() { Name = SchemesBox.SelectedItem.ToString(), Trainer = TrainerPathComboBox.SelectedItem.ToString(), Stream = ((DatabaseStream) StreamsBox.SelectedItem).Name, Annotator = AnnotatorsBox.SelectedItem.ToString() };

            if(fusionstreams.Find(item => item.Name == stp.Name) == null)
            {
                fusionstreams.Add(stp);
                SchemeandClassifierBox.Items.Add(stp);

                SchemeandClassifierBox.SelectAll();
            }
        ...
        public class SchemeTrainerPair
        {
            public string Name { get; set; }
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
                ...
                fusionstreams.Remove((SchemeTrainerPair) SchemeandClassifierBox.SelectedItem);
                SchemeandClassifierBox.Items.Remove(SchemeandClassifierBox.SelectedItem);
        ...
        private void SchemeandClassifierBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (SchemeandClassifierBox.SelectedItems != null)
            {
        ...
        private void NetworkBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
