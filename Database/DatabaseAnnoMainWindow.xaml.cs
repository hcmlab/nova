...
namespace ssi
{
    ...
    public partial class DatabaseAnnoMainWindow : Window
    {
        ...
        public List<DatabaseMediaInfo> GetMediaFromDB(string db, string session)
        {
            ...
            DatabaseHandler.GetObjectID(ref sessionID, DatabaseDefinitionCollections.Sessions, DatabaseHandler.SessionName);
            ...
            {
                DatabaseMediaInfo streamInfo = new DatabaseMediaInfo();
                string url = selectedmedia["url"].ToString();
                bool requiresAuth = selectedmedia["requiresAuth"].AsBoolean;
                ...
                streamInfo.connection = split[0];

                if (split[0] == "ftp" || split[0] == "sftp")
                {
                    string[] split2 = split[1].Split(new char[] { '/' }, 4);
                    streamInfo.ip = split2[2];


                    string filename = split2[3].Substring(split2[3].LastIndexOf("/") + 1, (split2[3].Length - split2[3].LastIndexOf("/") - 1));
                    streamInfo.folder = split2[3].Remove(split2[3].Length - filename.Length);
                }

                streamInfo.filepath = url;
                streamInfo.filename = selectedmedia["name"].ToString();
                streamInfo.requiresauth = requiresAuth;

                //Todo: solve references
                streamInfo.subject = selectedmedia["subject_id"].ToString();
                streamInfo.role = selectedmedia["role_id"].ToString();
                streamInfo.mediatype = selectedmedia["mediatype_id"].ToString();
                streamInfo.session = selectedmedia["session_id"].ToString();
                paths.Add(streamInfo);
            }
            return paths;
        }
        */
